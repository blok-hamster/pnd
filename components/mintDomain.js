...
export default function MintDomains() {
  ...
  useEffect(()=> {
    const provider = new ethers.providers.JsonRpcProvider(process.env.POLYGON_MUMBAI_ENDPOINT);
    const newSbtFactory = new ethers.Contract(config.sbtFactoryAddress, sbtDomainFactoryABi, provider);
    setSbtFactory(newSbtFactory);
    const newDomainFactory = new ethers.Contract(config.domainFactoryAddress, picardyDomainFactoryABi, provider);
  ...
  const mintSbtDomain = async (event) => {
    ...
    const sbtDomainContract = new ethers.Contract(tldAddress, sbtDoaminAbi, signer);
